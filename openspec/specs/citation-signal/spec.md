# citation-signal Specification

## Purpose

Provide a citation-count signal for cached papers — fetched from an external citation provider (Semantic Scholar by default), refreshed on a schedule, and used to dynamically re-score and re-rank papers as their real-world impact emerges. The signal is opt-in via a top-level `citations` config block; when disabled, the pipeline behaves identically to pre-citation behavior.
