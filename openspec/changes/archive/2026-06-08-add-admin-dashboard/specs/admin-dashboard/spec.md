...
The system SHALL serve `GET /admin/_system` as an HTML fragment summarizing the runtime state of the deployment, including: scoring model name, ingest interval, digest hour and timezone, SMTP host (NOT credentials), database path and on-disk size in bytes, the most recent `scored_at` timestamp from `papers` (used as a proxy for last successful ingest), the most recent `sent_at` timestamp from `sent_papers` (last successful digest), the count of active subscriptions, and the count of users currently loaded in the runtime `AppConfig.users` (so the operator can detect a load discrepancy).

#### Scenario: Renders without crashing on empty DB
- **WHEN** the panel loads against a freshly-created database with no papers and no sent rows
- **THEN** the response is `200 OK` and renders `—` (or equivalent) for the "last ingest" and "last digest" fields
